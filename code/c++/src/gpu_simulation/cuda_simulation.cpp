#include "../pucp_neutrino/gpu/probabilities.h"
#include "../pucp_neutrino/gpu/hutils.h"
...
void cuda_simulation_InvisibleDecay(
	...
	cudaMemcpy(ddm, _dm, 2*sizeof(double), cudaMemcpyHostToDevice);
	cudaMemcpy(dU1, U1, 9*sizeof(cuDoubleComplex), cudaMemcpyHostToDevice);
	cuda_InvisibleDecay(dU1, N, _sg, _L, _rho, ddm, dalpha);
	// cudaFree(denergy);
	cudaFree(devents);
	cudaFree(dalpha);
	cudaFree(ddm);
...
void cuda_simulation_NonStandardInteraction(
	...
	printf("Assigning memory on device.\n");
	// cudaMalloc(reinterpret_cast<void **>(&denergy), N*sizeof(double));
	cudaMalloc(reinterpret_cast<void **>(&dalpha), 9*sizeof(double));
	cudaMalloc(reinterpret_cast<void **>(&ddm), 2*sizeof(double));
	cudaMalloc(reinterpret_cast<void **>(&dU1), 9*sizeof(cuDoubleComplex));
	// printf("Copying values of energy from host to device.\n");
	// cudaMemcpy(denergy, henergy, N*sizeof(double), cudaMemcpyHostToDevice);
	cudaMemcpy(dalpha, _alpha, 9*sizeof(double), cudaMemcpyHostToDevice);
	cudaMemcpy(ddm, _dm, 2*sizeof(double), cudaMemcpyHostToDevice);
	cudaMemcpy(dU1, U1, 9*sizeof(cuDoubleComplex), cudaMemcpyHostToDevice);
	cuda_NonStandardInteraction(dU1, N, _sg, _L, _rho, ddm, dalpha);
	// cudaFree(denergy);
	// cudaFree(devents);
	cudaFree(dalpha);
	cudaFree(ddm);
...
void cuda_simulation_ViolationEquivalence(
	...
	cudaMemcpy(ddm, _dm, 2*sizeof(double), cudaMemcpyHostToDevice);
	cudaMemcpy(dU1, U1, 9*sizeof(cuDoubleComplex), cudaMemcpyHostToDevice);
	cuda_ViolationEquivalence(dU1, N, _sg, _L, _rho, ddm, dalpha);
	// cudaFree(denergy);
	// cudaFree(devents);
	cudaFree(dalpha);
	cudaFree(ddm);
