...
__global__ void gpu_nsi(
	...
	double* _dm, double* _alpha, cuDoubleComplex _I, cuDoubleComplex** _batchedDM,
	cuDoubleComplex** _batchedPot, cuDoubleComplex** _batchedU,
	cuDoubleComplex** _batchedNSI ){
	...
	if(tid < _size_data){
		...
		_batchedNSI[tid][0] = {_alpha[0], 0};
		_batchedNSI[tid][1] = cuCmul( 
			make_cuDoubleComplex(_alpha[3], 0), 
			exp(cuCmul(cuCmul(_I, make_cuDoubleComplex(-1,0)), make_cuDoubleComplex(_alpha[4], 0)) )
		);
		_batchedNSI[tid][2] = cuCmul(
			make_cuDoubleComplex(_alpha[5], 0),
			exp(cuCmul(_I, make_cuDoubleComplex(_alpha[6], 0)) )
		);
		_batchedNSI[tid][3] = cuCmul(
			make_cuDoubleComplex(_alpha[3], 0),
			exp(cuCmul(_I, make_cuDoubleComplex(_alpha[4], 0)))
		);
		_batchedNSI[tid][4] = make_cuDoubleComplex(_alpha[1], 0);
		_batchedNSI[tid][5] = cuCmul( 
			make_cuDoubleComplex(_alpha[7], 0),
			exp(cuCmul(cuCmul(_I, make_cuDoubleComplex(-1,0)),  make_cuDoubleComplex(_alpha[8], 0)) )
		);
		_batchedNSI[tid][6] = cuCmul( 
			make_cuDoubleComplex(_alpha[5], 0),
			exp(cuCmul(_I,  make_cuDoubleComplex(_alpha[6], 0)) )
		);
		_batchedNSI[tid][7] = cuCmul( 
			make_cuDoubleComplex(_alpha[7], 0),
			exp(cuCmul(_I,  make_cuDoubleComplex(_alpha[8], 0)) )
		);
		_batchedNSI[tid][8] = make_cuDoubleComplex(_alpha[2], 0);
	}
}


__global__ void sum_pot2hff4NSI_batched(
	cuDoubleComplex** Pot, cuDoubleComplex** Hff2, cuDoubleComplex** Hff3, cuDoubleComplex** _batchedNSI, cuDoubleComplex * _d_A, double _rho, int _batch_count) {
	...
	if (tid < _batch_count) {
		for (int i=0; i<9; i++) {
				Hff3[tid][i].x = Hff2[tid][i].x + Pot[tid][i].x + _rho*7.63247*0.5*1.e-14* _batchedNSI[tid][i].x;
				Hff3[tid][i].y = Hff2[tid][i].y + Pot[tid][i].y + _rho*7.63247*0.5*1.e-14* _batchedNSI[tid][i].y;
				_d_A[tid * 9 + i].x =  Hff3[tid][i].x;
				_d_A[tid * 9 + i].y =  Hff3[tid][i].y;
		}
	}
}


__global__ void building_SandV4NSI(
...
__global__ void buildP4NSI(double** _batchedP, cuDoubleComplex** _batchedS, int _batch_count){
	int tid = threadIdx.x + blockIdx.x*blockDim.x;
	if (tid < _batch_count){
		for (int i=0; i<3; i++){
			for (int j=0; j<3; j++){
...
void cuda_NonStandardInteraction(
	...
	double* _dm, double* _alpha) {
		...
		data_type** batchedNSI= nullptr;
		data_type* host_batchedNSI[_batch_count];
		data_type* device_batchedNSI[_batch_count];
		...
		for (int i = 0; i < _batch_count; i++) {
			...
			cudaMallocHost(&host_batchedNSI[i], sizeof(data_type)*m*n);
		...
		for (int i = 0; i < _batch_count; i++) {
			...
			cudaMalloc(reinterpret_cast<void **>(&device_batchedNSI[i]),	sizeof(data_type) * m*n);
		...
		cudaMalloc(reinterpret_cast<void **>(&batchedNSI), 	_batch_count*sizeof(data_type *));
		...
		cudaMemcpy(batchedNSI, device_batchedNSI, sizeof(data_type*)*_batch_count, cudaMemcpyHostToDevice);
		...
		gpu_nsi<<<blocks, threads>>>(
			...
			batchedDM, batchedPot, batchedU, batchedNSI);
		...
		sum_pot2hff4NSI_batched<<<blocks, threads>>> (batchedPot, batchedHff2, batchedHff3, batchedNSI, d_A, _rho, _batch_count);
		...
		building_SandV4NSI<<<blocks, threads>>> (
		...
		buildP4NSI<<<blocks, threads>>>(batchedP, batchedS2, _batch_count);
		...
		for (int i = 0; i < _batch_count; i++) {
			...
			cudaMemcpy(host_batchedNSI[i], device_batchedNSI[i], sizeof(data_type)* m*n, cudaMemcpyDeviceToHost );
		...
		for (int i=0; i < _batch_count; i++) {
			...
			printf("NSI[%d]:\n",i);
			print_matrix(m, n, host_batchedNSI[i], lda);
		// 	printf("Hff[%d]:\n",i);
		...
		for (int i = 0; i < _batch_count; i++) {
			...
			cudaFreeHost(host_batchedNSI[i]);
