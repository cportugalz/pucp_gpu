...
__global__ void gpu_violation_equivalence(
	...
	if(tid < _size_data){
		...
		_batchedDM[tid][0] = {2 * energy * _alpha[0] / energy,0};
		_batchedDM[tid][4] = {0.5 * _dm[0] / energy + 2 * energy * _alpha[1] , 0};
		_batchedDM[tid][8] = {0.5 * _dm[1] / energy + 2 * energy * _alpha[2], 0};
...
__global__ void sum_batched4VEP(
...
__global__ void building_SandV4vep(
...
__global__ void buildP4vep(double** _batchedP, cuDoubleComplex** _batchedS, int _batch_count){
	int tid = threadIdx.x + blockIdx.x*blockDim.x;
	if (tid < _batch_count){
		for (int i=0; i<3; i++){
			for (int j=0; j<3; j++){
...
void cuda_ViolationEquivalence(
	...
	double* _dm, double* _alpha) {
		...
		printf("Assigning host memory for Violation Equivalence Principle.\n");
		...
		printf("Assigning device memory for Violation Equivalence Principle.\n");
		for (int i = 0; i < _batch_count; i++) {
		...
		printf("Calling Violation Equivalence Principle Kernel with %d blocks and %.0f threads per block\n", blocks, threads);
		gpu_violation_equivalence<<<blocks, threads>>>(
		...
		sum_batched4VEP<<<blocks, threads>>> (batchedPot, batchedHff2, batchedHff3, d_A, _batch_count);
		cudaDeviceSynchronize();
		...
		building_SandV4vep<<<blocks, threads>>> (
		...
		buildP4vep<<<blocks, threads>>>(batchedP, batchedS2, _batch_count);
